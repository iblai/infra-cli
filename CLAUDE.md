...
2. **Platform Configuration** — domain selection (ingress picker if entries exist, otherwise free-text), prod-images release tag (the iblai-cli-ops tag is auto-resolved from prod-images' `[tool.uv.sources]` pin after credentials are collected; prompts only if the pin is unreadable)
