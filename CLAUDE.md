...
- `ibl_call` role runs: `ibl config save --set BASE_DOMAIN=…` → `ibl config environment call-only` → ECR login → `ibl call start` → `ibl call show-call-secrets` (printed to operator terminal, never persisted locally).
