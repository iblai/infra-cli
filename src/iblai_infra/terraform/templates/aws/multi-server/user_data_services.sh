#!/bin/bash
# ibl.ai Infrastructure -- Services Server Bootstrap Script
# Ubuntu 22.04 LTS

set -euo pipefail

exec > >(tee /var/log/user-data.log) 2>&1
echo "=== Services server bootstrap started at $(date) ==="
...
# Firewall — services server only needs internal traffic
# SSH is allowed via security group from VPN
ufw allow 22/tcp
...
[Unit]
Description=IBL Application (Services Server)
...
ExecStart=/bin/echo "IBL Services Server -- not yet configured"
Restart=always
RestartSec=10
...
echo "=== Services server bootstrap completed at $(date) ==="
